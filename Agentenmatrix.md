...
- **Live-Update 2026-05-11:** Server 2 wurde auf einen einzelnen technischen Gateway unter dem Linux-Nutzer `agent` bereinigt, wodurch der Telegram-`409 getUpdates`-Konflikt verschwand. Nach Operator-Klarstellung ist `agent` **kein echter Agentenname**. `Turiya` auf Server 3 nutzt jetzt Perplexity als Web-Search-Layer und wurde durch Deaktivierung von `acpx` und `bonjour` lokal stabilisiert.
...
| Server 2 | `sarahserver2` | `89.116.39.197` | 1 technische Instanz | 0 live | unter Linux läuft nur noch der technische Nutzer `agent`; das ist ausdrücklich kein echter Agentenname und nicht `Turiya` |
...
| Server 2 | `agent` | kein gültiger Agentenname, nur technischer Nutzer | aktiv | `Sarah` | Struktur, Orientierung, entwicklungsorientierte Begleitung | warm, klar, geerdet, integrativ | `~/.openclaw/workspace` | ja | teilweise | `openai-codex/gpt-5.4` | `gpt-5.4` über `openai-codex` | früherer Turiya-Bezug ist verworfen, alter User-Gateway am 2026-05-11 deaktiviert, Telegram-409 bereinigt |
...
### Server 2
- Aktive technische Instanz: Linux-Nutzer `agent`
...
- Operator-Klarstellung: Es gibt auf Server 2 **keinen echten Agenten namens `agent`**; der Name ist nur ein technischer Rest
