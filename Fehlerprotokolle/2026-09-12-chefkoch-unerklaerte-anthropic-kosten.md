...
7. Auf Slartis Auftrag wurde im botrepo der Branch `cheko_debugging` angelegt. Commit `4b809f70` fuehrt `agentctl models status|dryrun|apply` ein: Sol fuer Hauptagent/Heartbeat/System-Agent, Sonnet 4.6 fuer isolierte Subagenten, keine Modell-Fallbacks und Auth-Preflight fuer beide Provider. Der erste Live-Schema-Dry-run erkannte folgenlos einen falsch verschachtelten `systemAgent`-Block; nach Korrektur war der Dry-run auf S1 erfolgreich. 42 Unittests und der Ansible-Syntaxcheck sind erfolgreich. Chefkochs laufende Konfiguration wurde dabei nicht veraendert.
8. Nach erfolgreicher manueller OpenAI-OAuth-Anmeldung wurde Cheko am 2026-09-13 um 11:52 UTC live umgestellt: `openai/gpt-5.6-sol` fuer Hauptagent und Heartbeat, `openai/gpt-5.6-luna` als einziger Hauptmodell-Fallback und `anthropic/claude-sonnet-4-6` ohne Fallback fuer isolierte Subagenten. Telegram erlaubt nun Sarah und Slarti. Vorheriges `openclaw.json` wurde gesichert, die neue Config validiert und nur Chekos Gateway neu gestartet; Dienst anschliessend `active`, `NRestarts=0`. Botrepo-Branchstand: Commit `21a1b637`.
